...
public class HermesNodeInserter implements BatchNodeInserter {
	...
	public void insertBatch(TGraph graph, List<TPartStoredProcedureTask> tasks) {
		...
		overloadedThreshold = (int) Math
				.ceil(((double) tasks.size() / partMgr.getCurrentNumOfParts()) * (IMBALANCED_TOLERANCE + 1));
		if (overloadedThreshold < 1) {
			overloadedThreshold = 1;
		}
		List<TxNode> candidateTxNodes = findTxNodesOnOverloadedParts(graph, tasks.size());

		// System.out.println(String.format("Overloaded threshold is %d (batch size:
		// %d)", overloadedThreshold, tasks.size()));
		// System.out.println(String.format("Overloaded machines: %s, loads: %s",
		// overloadedParts.toString(), Arrays.toString(loadPerPart)));
		...
		while (!overloadedParts.isEmpty()) {
			// System.out.println(String.format("Overloaded machines: %s, loads: %s,
			// increaseTolerence: %d", overloadedParts.toString(),
			// Arrays.toString(loadPerPart), increaseTolerence));
		...
		// System.out.println(String.format("Final loads: %s",
		// Arrays.toString(loadPerPart)));
	}
	...
	private List<TxNode> findTxNodesOnOverloadedParts(TGraph graph, int batchSize) {
		...
		// Reverse the list, which makes the tx node ordered by tx number from large to
		// small
		...
		for (TxNode node : candidateTxNodes) {
			...
			for (int partId = 0; partId < partMgr.getCurrentNumOfParts(); partId++) {
				...
				if (delta <= increaseTolerence) {
					...
					if ((delta < bestDelta) || (delta == bestDelta && loadPerPart[partId] < loadPerPart[bestPartId])) {
						bestDelta = delta;
						bestPartId = partId;
					}
				}
			...
			// System.out.println(String.format("Find a better partition %d for tx.%d",
			// bestPartId, node.getTxNum()));
